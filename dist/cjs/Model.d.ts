...
type PickRequired<T extends OneModelSchema> = 
    Pick<T, { [K in keyof T]: T[K]["required"] extends true ? K : never }[keyof T]>

type PickOptional<T extends OneModelSchema> = 
Pick<T, { [K in keyof T]: T[K]["required"] extends true ? never : K }[keyof T]>

type UniqueKeys<T extends OneModelSchema> = 
{ [K in keyof T]: T[K]["unique"] extends true ? K : never }[keyof T]
...
Omit<
OptionalFields<PickOptional<T>>
& RequiredFields<PickRequired<T>>
  & {
    indexDefinitions: IndexMap,
    uniqueKeys: UniqueKeys<T>
    }
  , IndexKeys>;/*
  Model.d.ts -- Hand crafted type definitions for Model
...
export type HasUniqueKeys<T> = { uniqueKeys: keyof T }

export class Model<T extends HasIndexDefinitions<T> & HasUniqueKeys<T>, P = Omit<T, "indexDefinitions"  | "uniqueKeys">> {
  ...
  remove<U extends ExtractIndex<T, IndexName>, IndexName extends keyof T["indexDefinitions"] = 'primary', >(properties: Pick<T, T["uniqueKeys"]> & Omit<U, "indexDefinitions">, params?: OneParamsTyped<IndexName>): Promise<void>;
...
export class Model<T extends HasIndexDefinitions<T> & HasUniqueKeys<T>, P = Omit<T, "indexDefinitions">> {
    constructor(table: any, name: string, options?: ModelConstructorOptions);
    create(properties: P, params?: OneParams): Promise<P>;
    find<U extends ExtractIndex<T, IndexName>, IndexName extends keyof T["indexDefinitions"] = 'primary', >(properties: Omit<U, "indexDefinitions">, params?: OneParamsTyped<IndexName>): Promise<Paged<T[]>>;
    get<U extends ExtractIndex<T, IndexName>, IndexName extends keyof T["indexDefinitions"] = 'primary', >(properties: Omit<U, "indexDefinitions">, params?: OneParamsTyped<IndexName>): Promise<T>;
